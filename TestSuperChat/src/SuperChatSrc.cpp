...
/*Ncurses variables*/
WINDOW *win, *menu_window, *chat_window, *chat_clients_window, *input_window;
int width;
int height;
int cur_len = 0;
bool stillRunning = 1;
...
void loop();
void resetInputW();
char* get_string(const char*, char*, int);
    ...
    for (DDS::ULong j = 0; j < messageList.length(); j++)
    {
      ...
      for(int i = 0; i < numOfUsers; i++)
      {
        if(listOfUsers[i].uuid == messageList[j].uuid)
        {
          ...
          wmove(chat_window, yOffset, 1);
          waddstr(chat_window, listOfUsers[i].nick);
          waddstr(chat_window, ": ");
          waddstr(chat_window, messageList[j].message);
          wrefresh(chat_window);
...
void pub(char *msg)
{
  ...
  strncpy(messageInstance.message, msg, MESSAGE_SIZE_MAX);
    ...
    if(check == 50)
    {
      ...
      for(int i = 0; i < numOfUsers; i++)
      {
        if(listOfUsers[i].online && ((timeWatchingUsers - 5) >= listOfUsers[i].lastHeard))
        {
          ...
          wmove(chat_window, yOffset, 1);
          waddstr(chat_window, listOfUsers[i].nick);
          waddstr(chat_window, " Has Gone Offline");
          wrefresh(chat_window);
        }
      }
    }
...
void initializeLocalUser()
{
  ...
  if(file.fail())
  {
      ...
      resetInputW();
      wprintw(input_window, "%s", "Enter a User Nick: ");
      wgetnstr(input_window, tempNick, NICK_SIZE_MAX);
...
int main()
{
  ...
  pthread_create(&gui, NULL, initGUI, (void *) &trash);
...
void *initGUI(void* trash)
{
  initscr();
  start_color();
  cbreak();
  noecho();
  getmaxyx(stdscr, d_row, d_col);
  ...
  input_window = newwin(5, d_col, d_row - 5, 0);
    box(input_window, 0, 0);
    wrefresh(input_window);
  menu_window = newwin(3, d_col, 0, 0);
    box(menu_window, 0, 0);
    wrefresh(menu_window);
  chat_clients_window = newwin(d_row - 8, 25, 3, d_col - 25);
    box(chat_clients_window, 0, 0);
    wrefresh(chat_clients_window);
  chat_window = newwin(d_row - 8, d_col - 25, 3, 0);
    box(chat_window, 0, 0);
    wrefresh(chat_window);
...
void loop() 
{
  char temp_buffer[MESSAGE_SIZE_MAX];
  ...
  while(stillRunning) 
  {
    ...
    {
      resetInputW();
      wprintw(input_window, "%s", "Message: ");
      wgetnstr(input_window, temp_buffer, MESSAGE_SIZE_MAX);
...
char* get_string(const char * form, char * dst, int max_len) 
{
  int cur_len = 0;
