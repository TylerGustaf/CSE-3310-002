    ...
    struct _Chat_ChatMessage *to)
{
    ...
    if(from->happy){
        to->happy = c_stringNew(base, from->happy);
    } else {
        OS_REPORT (OS_ERROR, "copyIn", 0,"Member 'Chat::ChatMessage.happy' of type 'c_string' is NULL.");
        result = OS_C_FALSE;
    }
#else
    to->content = c_stringNew(base, from->content);
    ...
    struct _Chat_NamedMessage *to)
{
    ...
    if(from->happy){
        ...
        OS_REPORT (OS_ERROR, "copyIn", 0,"Member 'Chat::NamedMessage.happy' of type 'c_string' is NULL.");
        result = OS_C_FALSE;
    }
#else
    to->content = c_stringNew(base, from->content);
    ...
    void *_to)
{
    ...
    to->happy = DDS::string_dup(from->happy ? from->happy : "");
}
