...
int HelloWorldDataPublisher(int argc, char *argv[])
{
  ...
  mgr.registerType(messageTS.in());

  //create Topic
  char topic_name[] = "SuperChat_message";
  ...
  messageDataWriter_var messageWriter = messageDataWriter::_narrow(dwriter.in());

  message messageInstance; /* Example on Stack */
  messageInstance.uuid = 1;
char input[] = "Hello World Test";
for(int i = 0; i < 144; i++)
{
  messageInstance.message[i] = input[i];
}
  //messageInstance.message = DDS::string_dup("Hello World");
  cout << "=== [Publisher] writing a message containing :" << endl;
  cout << "    userID  : " << messageInstance.uuid << endl;
  cout << "    Message : \"" << messageInstance.message << "\"" << endl;

  ReturnCode_t status = messageWriter->write(messageInstance, DDS::HANDLE_NIL);
